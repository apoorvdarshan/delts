...
private struct ProfileEditorView: View {
    ...
    private var durationRangeBinding: Binding<WorkoutDurationRangeOption> {
        Binding {
            WorkoutDurationRangeOption.matching(minutes: profile.workoutDurationMinutes)
        } set: { newValue in
            profile.workoutDurationMinutes = newValue.targetMinutes
            profile.updatedAt = Date()
        }
    }
...
private struct ProfileMultiSelectMenuRow<Option: Hashable>: View {
        ...
        VStack(alignment: .leading, spacing: 0) {
            ProfileFieldRow(title: title, systemImage: systemImage) {
                Menu {
                    ForEach(options, id: \.self) { option in
                        Button {
                            selection = toggledSelection(option)
                        } label: {
    ...
    private func toggledSelection(_ option: Option) -> Set<Option> {
        var next = selection
